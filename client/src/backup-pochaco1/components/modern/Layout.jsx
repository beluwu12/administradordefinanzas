...
const Sidebar = ({ isOpen, onClose }) => {
    ...
    return (
        <>
            ...
            <aside className={`
                flex flex-col w-64 h-full border-r border-card-border bg-bg-sidebar flex-shrink-0
    ...
    return (
        <header className="h-16 border-b border-card-border bg-bg-sidebar/80 backdrop-blur-md flex items-center justify-between px-6 sticky top-0 z-20">
            ...
            <div className="flex items-center gap-4 md:hidden">
                <button onClick={onMenuClick} className="text-text-main p-1 -ml-1">
            ...
            <div className="flex items-center gap-4">
                ...
                <Link
                    to="/settings"
                    className="p-2 text-text-muted hover:text-text-main transition-colors hidden md:block"
                    title="Configuración del Perfil"
                >
                    <span className="material-symbols-outlined">settings</span>
                </Link>

                <div className="h-8 w-px bg-card-border mx-1 hidden md:block"></div>
...
const Layout = ({ children, title = 'Panel', subtitle }) => {
    ...
    return (
        <div className="bg-bg-body font-display text-text-main antialiased selection:bg-primary selection:text-white h-screen overflow-hidden flex">
            ...
            <main className="flex-1 flex flex-col min-w-0 bg-bg-body relative md:ml-0">
                <Header
                    title={title}
